...
trait EvenMoreSugar extends MockitoSugar {
  ...
  class Stubbed[T](c: => T) {
    ...
    def r[V](f: InvocationOnMock => V, g: V => T)(i: InvocationOnMock) = g(f(i))

    def answersWithArgument[V](position: Int = 0, c: Class[V], f: V => T) = answers(r(getArgument(c, position), f))
  ...
  def reset[T](mocks: T*) = Mockito.reset(mocks : _*)
}
